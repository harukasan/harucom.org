---
layout: home
lang: en
ref: home
permalink: /en/
---

Harucom is a handmade single-board computer you can get for about $20.
You can connect a keyboard and a TV directly, so you can experience Ruby programming without using a PC.

The name "Harucom" carries several meanings:

- **H**euristically **A**ccessible **Ru**by **Com**puter
- a **Ha**ndmade **Ru**by **Com**puter
- **Haru**kasan's Ruby **Com**puter

Harucom aims to be an accessible computer where you can learn programming intuitively.
By removing the complexities of modern computing, the goal is to let you touch the essence of programming.

## Learn how to use Harucom

* [Getting Started](/en/docs/getting-started/)
* [Programming Reference](/en/docs/reference/)
* [PicoRabbit](/en/docs/picorabbit/)
* [Upgrade Harucom OS](/en/flash/)

## Get a Harucom Board

The Harucom Board is available on BOOTH.
You can also manufacture one yourself from the published board data.

* [Harucom board - harukasan's BOOTH](https://harukasan.booth.pm/items/8127045)

For a fuller description of the Harucom Board, see the following pages.

- [Harucom Bundle with acrylic panels](/en/docs/harucom-bundle/)
- [Harucom Board](/en/docs/harucom-board/)

## Learn about Harucom

Harucom is an open-source project.
The board data is licensed under CERN-OHL-P v2, and the source code is licensed under the MIT License.

- [Harucom-board](https://github.com/harukasan/harucom-board/)
- [Harucom-OS](https://github.com/harukasan/harucom-os/)

I will be giving a talk about this project at RubyKaigi 2026.

- [Building a Standalone Ruby Programming Environment - RubyKaigi 2026](https://rubykaigi.org/2026/presentations/harukasan.html)
