...
| `/etc/` | 設定ファイルを置く場所です。[設定](../settings/)はここの `env.yml` で変更します。 |
| `/data/` | ドラムの音などのデータファイルが入っています。 |
| `/system.rb` | Harucom の起動時に実行されるスクリプトです。 |

Harucom ではどのディレクトリにもファイルを配置できます。
ルートディレクトリ `/` に自分のスクリプトやデータファイルを自由に保存して構いません。

システムが使用するスクリプトを書き換えた場合、その内容は電源を切っても残ります。
[ファームウェアを更新](/flash/)したときに、システムのファイルは初期状態に書き戻されます。
すべてを初期状態に戻したいときは、[ファイルシステムの初期化](../settings/#ファイルシステムを初期化する)を行ってください。
...
`ls` や `cp` など、ファイルを扱うコマンドは[コマンド一覧](../commands/)にまとめてあります。
