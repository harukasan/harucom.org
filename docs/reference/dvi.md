...
`DVI::Text.cols` と `DVI::Text.rows` を使います。
...
```ruby
DVI::Text.set_resolution(320, 240)   # 53桁 x 18行（文字が2倍の大きさになる）
DVI::Text.set_resolution(640, 480)   # 106桁 x 37行に戻す
```

テキストモードの解像度を切り替えます。指定できるのは 640x480 と 320x240 の2つです。
320x240 ではピクセルが2倍に拡大されて表示されるので、文字が大きく読みやすくなります。

切り替えは垂直同期のタイミングで反映され、戻ってきたときには `cols` と `rows` は
新しい値になっています。画面の内容は引き継がれないので、切り替えたあとに描き直してください。

IRB の画面の大きさを変えるだけなら [`zoom` コマンド](../../settings/#画面をズームする)が使えます。
...
[P5 描画ライブラリ](../p5/) の `color(r, g, b)` メソッドを使うと、通常の RGB 値（0〜255）
から RGB332 に変換できます。
