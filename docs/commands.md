...
Harucom の IRB では、ファイルを見たりコピーしたりするためのコマンドを実行できます。
...
これらのコマンドの正体は `/app` に置かれた Ruby スクリプトです。
中身を `cat` コマンドで表示したり、[`edit` コマンド](../app/edit/)で編集することもできます。
...
いまいるディレクトリのファイルを一覧表示します。
ディレクトリはファイルと別の色で表示されます。
...
現在開いているディレクトリ(カレントディレクトリ)を移動します。
ディレクトリを移動すると  `ls` や `cat`、`edit` にファイル名だけを渡したとき、そのディレクトリのファイルを差します。
...
止めたまま IRB に戻れます。`fg` と入力すると続きから再開します。
